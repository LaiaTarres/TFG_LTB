...
def train_model(model, criterion, optimizer, scheduler, num_epochs, dataloaders, dataset_sizes, class_names):
    since = time.time()
...
def main():
    ...
    train_dataset = datasets.ImageFolder(os.path.join(data_dir, 'train'),
                                         data_transforms['train'])  # mirar si ho fa random
    val_dataset = datasets.ImageFolder(os.path.join(data_dir, 'val'), data_transforms['val'])

    print('Classes to index', train_dataset.class_to_idx)

    weights_train = make_weights_for_balanced_classes(train_dataset.imgs, len(
        train_dataset.classes))  # crees els weights per les images i les classes
    weights_train = torch.FloatTensor(weights_train)  # n-dimensional array, és més fàcil per operar que una matriu
    sampler_train = torch.utils.data.sampler.WeightedRandomSampler(weights_train, len(
        weights_train))  # agafa mostres amb la probabilitat de weights_train
    ...
    train_dataloader = DataLoader(train_dataset, batch_size=36, num_workers=4,
                                  sampler=sampler_train)  # combina un dataset i un sampler. Agafa dades del train set, la mida del batch, el nombre de subprocessos per carregar les dades i el sampler (quines dades s'han d'agafar)
    ...
    model_ft = train_model(model_ft, criterion, optimizer_ft, exp_lr_scheduler,
                       num_epochs=70, dataloaders, dataset_sizes, class_names) #s'entrena el model amb la resnet18,

    torch.save(model_ft, 'output_model_3.pth') #canviar això per saber quin és

if __name__ == "__main__":
    main()
#a l'entrar: source VirtualEnvs/venv/bin/activate
#source /imatge/ltarres/VirtualEnvs/venv/bin/activate
# module load python/3.6.2 cuda/8.0
#cd PycharmProjects/test/
# per correr srun --mem 16GB --gres=gpu:1 python main_marcv1.py #el sergio diu de tambe afegir-li --mem 16GB i provar si així funciona...
