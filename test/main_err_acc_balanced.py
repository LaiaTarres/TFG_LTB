...
learning_rate=float('1e-'+str(sys.argv[3]))
...
escriptura_loss_val = '/imatge/ltarres/PycharmProjects/test/loss_acc/val_loss_'+model+'_'+str(epochs)+'ep_lr'+str(sys.argv[3])+a_learning_rate+'_bs'+str(batch_size)+'_balanced.txt'
escriptura_loss_train = '/imatge/ltarres/PycharmProjects/test/loss_acc/train_loss_'+model+'_'+str(epochs)+'ep_lr'+str(sys.argv[3])+a_learning_rate+'_bs'+str(batch_size)+'_balanced.txt'
escriptura_acc_train = '/imatge/ltarres/PycharmProjects/test/loss_acc/train_acc_'+model+'_'+str(epochs)+'ep_lr'+str(sys.argv[3])+a_learning_rate+'_bs'+str(batch_size)+'_balanced.txt'
escriptura_acc_val = '/imatge/ltarres/PycharmProjects/test/loss_acc/val_acc_'+model+'_'+str(epochs)+'ep_lr'+str(sys.argv[3])+a_learning_rate+'_bs'+str(batch_size)+'_balanced.txt'
...
weights_train = torch.FloatTensor(weights_train) #n-dimensional array, és més fàcil per operar que una matriu
...
#afegim això per fer la loss
weights_train_class=make_weights_per_class(train_dataset.imgs, len(train_dataset.classes))
weights_train_class = torch.FloatTensor(weights_train_class)

weights_val = make_weights_for_balanced_classes(val_dataset.imgs, len(val_dataset.classes)) #igual pero per val
weights_val = torch.FloatTensor(weights_val) #important canviem això de DoubleTensor a FloatTensor
sampler_val = torch.utils.data.sampler.WeightedRandomSampler(weights_val, len(weights_val))
#afegim això per fer la balanced loss
weights_val_class=make_weights_per_class(val_dataset.imgs, len(val_dataset.classes))
weights_val_class = torch.FloatTensor(weights_val_class)
    ...
    for epoch in range(num_epochs):
        ...
        for phase in ['train', 'val']:
            if phase == 'train':
                ...
                #posem el criterion de loss amb els weights de train
                criterion = nn.CrossEntropyLoss(weight=weights_train_class.cuda())
            else:
                model.eval()   # Set model to evaluate mode
                #posem el criterion de loss amb els weights de val
                criterion = nn.CrossEntropyLoss(weight=weights_val_class.cuda())

            running_loss = 0.0
            running_corrects = 0
            data_acc = torch.tensor([], dtype=torch.long, device='cuda:0') #les reals acumulades
            preds_acc = torch.tensor([], dtype=torch.long, device='cuda:0') #les prediccions acumulades
            ...
            epoch_balanced_acc = balanced_accuracy_score(data_acc.cpu(), preds_acc.cpu(), adjusted=True)

            print('{} Loss: {:.4f} Acc: {:.4f} Balanced Acc: {:.4f}'.format(phase, epoch_loss, epoch_acc, epoch_balanced_acc))
...
torch.save(model_ft, 'output_model_'+model+'_'+str(epochs)+'ep_lr'+str(sys.argv[3])+a_learning_rate+'_bs'+str(batch_size)+'_balanced.pth') #canviar això per saber quin és


#a l'entrar: source VirtualEnvs/venv/bin/activate
# module load python/3.6.2 cuda/8.0
#cd PycharmProjects/test/
# per correr srun --mem 16GB --gres=gpu:1 python main_err_acc_balanced.py #el sergio diu de tambe afegir-li --mem 16GB i provar si així funciona...
...
#una vegada modificat, s'executa: srun --mem 32GB --gres=gpu:1 python main_err_acc_balanced.py resnet18 100 6 a 36 -- crec que això se m'ha carregat els fitxers anteriors...

#srun --mem 32GB --gres=gpu:1 python main_err_acc_balanced.py resnet34 100 6 na 36

#mirar my_resnet.py de la tere,
#vas al model de la resnet i treus la fully connected de la ultima capa, i després fas model.extract_features() i et dona una matriu, per cada imatge del batch size et dóna les característiques.
