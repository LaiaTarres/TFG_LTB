...
def hardVotingClassifier(preds_model1, preds_model2, preds_model3, preds_model4):
    ...
    for i in range(len(preds_model1)): #range perquè li estavem tensor amb un vector de mida batch size, aquí ja li passarem un vector
        list_preds = [preds_model1[i], preds_model2[i], preds_model3[i], preds_model4[i]]#llista amb les tres posicions
...
def softVotingClassifier(probabilities1, probabilities2, probabilities3, probabilities4):
    ...
    probabilities4 = np.array(probabilities4)
    preds_ensemble=[]
    for i in range(probabilities1.shape[0]): #aquí et vindrà, per cada imatge, un vector
        list_probs=[]
        for j in range(probabilities1.shape[1]):
            list_probs.append((probabilities1[i][j]+probabilities2[i][j]+probabilities3[i][j]+probabilities4[i][j])/4)
...
def softVotingClassifierWeighted(probabilities1, w1, probabilities2, w2, probabilities3, w3, probabilities4, w4):
    """ Implements a voting classifier for pre-trained classifiers"""
    probabilities1 = np.array(probabilities1)
    probabilities2 = np.array(probabilities2)
    probabilities3 = np.array(probabilities3)
    ...
    for i in range(probabilities1.shape[0]): #aquí et vindrà, per cada imatge, un vector
        list_probs=[]
        for j in range(probabilities1.shape[1]):
            list_probs.append((w1*probabilities1[i][j]+w2*probabilities2[i][j]+w3*probabilities3[i][j]+w4*probabilities4[i][j])/4)
        #print('list_probs',list_probs)
        preds_ensemble.append(list_probs.index(max(list_probs))) #et torna la posició del màxim, s'ha de comprovar que vagi de 0 a 6
...
model3=sys.argv[3]
model4=sys.argv[4]
tipus_ensemble=sys.argv[5]
print('se li ha de passar el model sense pth [output_model_my_resnet_18_2018 output_model_my_resnet_34_2018 output_model_my_resnet_51_2018 model4 tipus_ensemble]', model1, model2, model3, model4, tipus_ensemble)
...
params4_train_file='model_output/train_params_'+model4+'.txt'
params4_val_file='model_output/val_params_'+model4+'.txt'
...
probs4_train_file='model_output/train_probabilities_'+model4+'.txt'
probs4_val_file='model_output/val_probabilities_'+model4+'.txt'
...
preds4_train_file='model_output/train_preds_'+model4+'.txt'
preds4_val_file='model_output/val_preds_'+model4+'.txt'
...
with open(params4_train_file, 'r') as textFile:
    params1t = textFile.readlines()
params4_train = [[float(num) for num in s.replace('\n', '').replace('[[','').replace(']]','').split(',')] for s in params1t]
with open(params4_val_file, 'r') as textFile:
    params1v = textFile.readlines()
params4_val = [[float(num) for num in s.replace('\n', '').replace('[[','').replace(']]','').split(',')] for s in params1v]
...
with open(preds4_train_file, 'r') as textFile: #ja està bé
  preds1t = textFile.readlines()
preds4_train = [float(s.replace('\n', '')) for s in preds1t]
with open(preds4_val_file, 'r') as textFile:
  preds1v = textFile.readlines()
preds4_val = [float(s.replace('\n', '')) for s in preds1v]
...
with open(probs4_train_file, 'r') as textFile:
  probs1t = textFile.readlines()
probs4_train = [[float(num) for num in s.replace('\n', '').replace('[[','').replace(']]','').split(',')] for s in probs1t]
with open(probs4_val_file, 'r') as textFile:
  probs1v = textFile.readlines()
probs4_val = [[float(num) for num in s.replace('\n', '').replace('[[','').replace(']]','').split(',')] for s in probs1v]
...
if tipus_ensemble=='hard_voting':
    preds_ensemble_train = hardVotingClassifier(preds1_train, preds2_train, preds3_train, preds4_train)
    preds_ensemble_val = hardVotingClassifier(preds1_val, preds2_val, preds3_val, preds4_val)
elif tipus_ensemble=='soft_voting':
    preds_ensemble_train = softVotingClassifier(probs1_train, probs2_train, probs3_train, probs4_train)
    preds_ensemble_val = softVotingClassifier(probs1_val, probs2_val, probs3_val, probs4_val)
elif tipus_ensemble=='soft_voting_weighted':
    w1=0.5166575701846157/(0.5166575701846157+0.515672380049196+0.5908988024458007 + 0.7525369892128912) #0.2
    w2=0.515672380049196/(0.5166575701846157+0.515672380049196+0.5908988024458007 + 0.7525369892128912) # 0.2
    w3=0.5908988024458007/(0.5166575701846157+0.515672380049196+0.5908988024458007 + 0.7525369892128912) #0.2
    w4=0.7525369892128912/(0.5166575701846157+0.515672380049196+0.5908988024458007 + 0.7525369892128912) #0.4
    preds_ensemble_train=softVotingClassifierWeighted(probs1_train, w1, probs2_train, w2, probs3_train, w3, probs4_train, w4)
    preds_ensemble_val=softVotingClassifierWeighted(probs1_val, w1, probs2_val, w2, probs3_val, w3, probs4_val, w4)
    ...
    preds_svm_val_4 = svm_prediction(params4_train, labels_train)
    preds_ensemble_val = hardVotingClassifier(preds_svm_val_1, preds_svm_val_2, preds_svm_val_3, preds_svm_val_4)
else :
...
balanced_acc_4_train=balanced_accuracy_score(labels_train, preds4_train, adjusted=True)
print('balanced_acc_4_train',balanced_acc_4_train)
...
balanced_acc_4_val=balanced_accuracy_score(labels_val, preds4_val, adjusted=True)
print('balanced_acc_4_val',balanced_acc_4_val)
